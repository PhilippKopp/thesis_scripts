...
OUTPUTPATH="/user/HS204/m09113/my_project_folder/Results/alpha_errors_KF-ITW_over_pose_thesis"
SAVE4PRES =None
SAVE4LATEX=None
...
GT_ALPHA_DIR = '/user/HS204/m09113/my_project_folder/KF-ITW-prerelease/'
GT_ALPHA_EXP_FITTING_LOG = '/multi_iter100_reg30/fitting.log'
...
for id_and_expression_dir in id_and_expression_dirs:
	...
	gt_alphas, _ = oal.read_fitting_log( GT_ALPHA_DIR + id_and_expression_dir.split('/')[-2] +"/"+ id_and_expression_dir.split('/')[-1][0:-5]+GT_ALPHA_EXP_FITTING_LOG)
	...
	for i in range(len(experiment_alphas)):
		...
		alpha_errors = mean_alphas- gt_alphas
		l2_error = np.linalg.norm(alpha_errors)
		experiment_errors.append(l2_error)
	all_errors.append(experiment_errors)
...
#	y_coordinates = np.mean(all_errors[:,:,alpha_idx], axis=0)  # [i[alpha_idx] for i in all_errors]
#	y_deviation   = np.std(all_errors[:,:,alpha_idx], axis=0)
...
if SAVE4PRES:
	plt.savefig("/user/HS204/m09113/my_project_folder/Results/alpha_errors_KF-ITW_over_pose_prepared"+".png")
