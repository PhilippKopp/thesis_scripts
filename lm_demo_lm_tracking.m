...
parfor i = 1:length(first_img_list)
    ...
    if write_pts
        mkdir ([first_img_list{i}(1:end-17) 'CSR_lms_lm_tracking']);
    end
    ...
    for frame_num = 1:length(all_vid_frames)
		...
		init_lmk = project_s2b(cr_model.mean_shape, bbox);
            ...
            % write lm files
            if write_pts
                ofid = fopen([first_img_list{i}(1:end-17) 'CSR_lms_lm_tracking/' all_vid_frames(frame_num).name(1:end-3) 'pts'], 'w');
