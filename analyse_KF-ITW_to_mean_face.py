...
for ID in IDS:
	for EXPRESSION in EXPRESSIONS:
		...
		mean_face = '/user/HS204/m09113/my_project_folder/surrey_mm_mean_shapes/mean_shape_3449.obj'

		gt_imp_vertices = oal.get_KF_ITW_vertex_ids(ID,EXPRESSION)
		if (gt_imp_vertices == None):
			continue

		experiment_folder = DB_FITS_BASE+ID+EXPRESSION+EXPERIMENT
		...
		#### Do the real work
		oal.register_and_align_KF_ITW_to_surrey(mean_face, gt_imp_vertices, gt_obj_model, registered_gt_obj_model, aligned_gt_obj_model, use_vertices=oal.get_lsfm_crop_mask_surrey_3448_vertices())
		
		distances = oal.measure_distances_on_surface_non_registered_pymesh( source_obj_file=aligned_gt_obj_model, destination_obj_file=mean_face, measure_on_source_vertices=oal.get_lsfm_crop_mask_surrey_3448_vertices())
		...
		# get eye positions
		eye_positions = oal.get_vertex_positions(mean_face, oal.surrey_outer_eye_vertices)
		...
		with open(analysis_log, "w") as analysis:
			...
			analysis.write("mean obj model: "+mean_face+"\n")
